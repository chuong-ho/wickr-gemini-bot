...
async function main() {
 ...
 // Initialize Gemini AI
    if (tokens.GEMINI_API_KEY) {
      genAI = new GoogleGenerativeAI(tokens.GEMINI_API_KEY.value);
    } else {
      exitHandler(null, { exit: true, reason: 'GEMINI_API_KEY not found!' });
    }

//Check if wickr client configured
if (process.argv[2] === undefined) {
      if (tokens.BOT_USERNAME !== undefined) {
        ...
        exitHandler(null, { exit: true, reason: 'Client username not found!' });
      }
      var status = await bot.start(bot_username)
    } else {
      var status = await bot.start(process.argv[2])
    }

    if (!status) {
      exitHandler(null, { exit: true, reason: 'Client not able to start' });
    }
  ...
  try {
    ...
    if (found === undefined) {
      var wickrUser = new WickrUser(userEmail, {
        index: 0,
        personal_vGroupID: convoType === 'personal' ? vGroupID : "",
        command: "",
        argument: ""
      });
    ...
    // Handle help command
    if (command === '/help') {
      var reply = "Gemini AI Chatbot Commands:\n";
      reply += "/help - Show this help message\n";
      reply += "Just type any message to chat with Gemini AI!";
    ...
    if (messageText && messageText.trim() !== '') {
      try {
        ...
        WickrIOAPI.cmdSendRoomMessage(vGroupID, text);
      } catch (error) {
        console.error('Gemini API error:', error);
        WickrIOAPI.cmdSendRoomMessage(vGroupID, "Sorry, I encountered an error processing your request.");
      }
    }
...
// The following code is example code provided by Wickr
